...
def match_line(pattern: Pattern[str], line: str, full_match: bool, invert_mode: bool) -> bool:
...
def match_lines(pattern: Pattern[str], data: List[str],
                full_match: bool, invert_mode: bool) -> List[str]:
...
def compile_pattern(pattern: str, is_regex: bool, ignore_case: bool) -> Pattern:
    if not is_regex:
        pattern = re.escape(pattern)
    return re.compile(pattern, flags=re.IGNORECASE) if ignore_case else re.compile(pattern)


def format_data(data: List[str], counting_mode: bool, only_files: bool, only_files_invert: bool,
                source_name: str = None) -> List[str]:
...
def find_in_source(source: Iterable, pattern: Pattern[str],
                   counting_mode: bool, full_match: bool,
                   invert_mode: bool, only_files: bool,
                   only_files_invert: bool, source_name: str = None) -> List[str]:
