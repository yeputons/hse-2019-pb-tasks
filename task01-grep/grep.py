...
def is_regex(needle: str, line: str) -> bool:
    """ Returns true if regex is found false otherwise """
    return re.search(needle, line)


def is_substring(needle: str, line: str) -> bool:
    """ Returns true if substring is found false otherwise """
    return needle in line


def find_in_input(count_mode: bool, regex_mode: bool, needle: str,
                  file: TextIO, file_name: str, number_of_input_data: int) -> None:
...
def find_in_files(args: argparse.Namespace) -> None:
    ...
    for current_file in args.files:
        with open(current_file, 'r') as in_file:
            find_in_input(args.count, args.regex, args.needle,
                          in_file, current_file, len(args.files))
...
def main(args_str: List[str]):
    ...
    if args.files:
        ...
        find_in_input(args.count, args.regex, args.needle, sys.stdin, '', 0)
