...
def test_unit_exclude_nonexistent_files(tmp_path, monkeypatch, capsys):
    (tmp_path / 'a.txt').write_text('something')
    (tmp_path / 'b.txt').write_text('something#2')
    monkeypatch.chdir(tmp_path)
    grep.exclude_nonexistent_files(['a.txt', 'not.txt', 'b.txt', 'b.txt', 'd.txt'])
    out, err = capsys.readouterr()
    assert err == 'No such file: not.txt\nNo such file: d.txt\n'
    assert out == ''


def test_unit_read_all_lines(tmp_path, monkeypatch, capsys):
    (tmp_path / 'a.txt').write_text('something\nhello\nhow are you?\n')
    (tmp_path / 'b.txt').write_text('interesting\nwhat is going on?\n\n')
    monkeypatch.chdir(tmp_path)
    read = grep.read_all_lines(['a.txt', 'b.txt'])
    out, err = capsys.readouterr()
...
def test_unit_filter_matched_lines(capsys):
    ...
    lines = grep.filter_matched_lines(lines, 'you', False)
    out, err = capsys.readouterr()
