...
def test_exists_in_line_empty():
    assert not grep.exists_in_line('some text', '', False)


def test_exists_in_line_regex():
    assert grep.exists_in_line('.o+', 'file.o', True)
    assert not grep.exists_in_line('.exe+', 'a.bin', True)


def test_search_in_lines_func():
    lines: List[str] = ['Line 1', 'Line 2', 'Line 3']
    assert grep.search_in_lines(lines, False, '2') == ['Line 2']


def test_search_in_lines_regex():
    lines: List[str] = ['Line 1', 'Line 2', 'Line 3']
    assert grep.search_in_lines(lines, True, 'i+n') == ['Line 1', 'Line 2', 'Line 3']


def test_search_in_lines_empty():
    lines: List[str] = []
    assert grep.search_in_lines(lines, False, 'pattern') == []
...
def test_print_line_func(capsys):
    ...
    out, err = capsys.readouterr()
    assert err == ''
    assert out == 'a.txt:Some line\n'


def test_print_result_function(capsys):
    lines: List[str] = ['Line 1']
    grep.print_results('a.txt', False, lines)
    out, err = capsys.readouterr()
    assert err == ''
    assert out == 'a.txt:Line 1\n'


def test_print_result_function_empty(capsys):
    lines: List[str] = []
    grep.print_results('', True, lines)
    out, err = capsys.readouterr()
    assert err == ''
    assert out == '0\n'
...
def test_print_result_function_count_len(capsys):
    lines: List[str] = ['line 1', 'line 2']
    grep.print_results('a.txt', True, lines)
    out, err = capsys.readouterr()
    assert err == ''
    assert out == 'a.txt:2\n'
...
def test_file_search_no_file(capsys):
    grep.find_pattern(['a.txt'], False, True, 'h+?')
    out, err = capsys.readouterr()
    assert err == ''
...
def test_files_search(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.chdir(tmp_path)
    grep.find_pattern(['b.txt'], False, False, 'stuff')
    out, err = capsys.readouterr()
    assert err == ''
...
def test_console_search_empty(monkeypatch, capsys):
    monkeypatch.setattr('sys.stdin', io.StringIO(''))
    out, err = capsys.readouterr()
    grep.find_pattern([], False, False, 'str')
    assert err == ''
    assert out == ''
