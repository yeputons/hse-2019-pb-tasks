...
def test_file_processing_lines(tmp_path, capsys):
    (tmp_path / 'a.txt').write_text('pref needle\nneedle suf\n')
    (tmp_path / 'b.txt').write_text('the needl\npref needle suf')
    with open('a.txt', 'r') as f:
        grep.processing_lines(argparse.Namespace(
...
def test_stdin_processing_lines(monkeypatch, capsys):
    monkeypatch.setattr('sys.stdin', io.StringIO(
        'pref needle?\nneedle? suf\nthe needl\npref needle? suf'))
    grep.processing_lines(argparse.Namespace(needle='needle', files=[], regex=False, cnt=False),
...
def test_empty_lines(tmp_path, capsys):
    (tmp_path / 'a.txt').write_text('pref needle\nneedle gref\n')
    with open('a.txt', 'r') as f:
        grep.processing_lines(argparse.Namespace(
            needle='if', files=['a.txt'], regex=False, cnt=False), f, 'a.txt')
    out, err = capsys.readouterr()
    assert err == ''
    assert out == ''
