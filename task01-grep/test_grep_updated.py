...
def test_unit_print_res(capsys):
    ...
    print_res(result, False, False, False, False)
    out, err = capsys.readouterr()
    assert err == ''
    assert out == 'a.txt:1\na.txt:2\nb.txt:1 in b\nb.txt:2 in b\n'
...
def test_unit_print_res_count(capsys):
    ...
    print_res(result, True, False, False, False)
    out, err = capsys.readouterr()
    assert err == ''
    assert out == 'a.txt:2\nb.txt:2\n'
...
def test_unit_print_res_only_files(capsys):
    ...
    print_res(result, False, True, False, False)
    out, err = capsys.readouterr()
    assert err == ''
    assert out == 'a.txt\nb.txt\nkavo.txt\n'


def test_unit_print_res_only_not_files(capsys):
    result = [('a.txt', []), ('b.txt', ['1b']), ('kavo.txt', [])]
    print_res(result, False, False, True, False)
    out, err = capsys.readouterr()
    assert err == ''
    assert out == 'a.txt\nkavo.txt\n'
...
def test_unit_print_res_stdin(capsys):
    ...
    print_res(result, False, False, False, True)
    out, err = capsys.readouterr()
    assert err == ''
    assert out == '1\ndva\nthree two two,..,\n'
