...
def test_x_wins_finished(mocker: pytest_mock.MockFixture) -> None:
    send_message = mocker.stub(name='send_message_stub')
    bot = TicTacToeUserHandler(send_message)
    send_messages(bot, [
        'start',
...
def test_o_wins_finished(mocker: pytest_mock.MockFixture) -> None:
    send_message = mocker.stub(name='send_message_stub')
    bot = TicTacToeUserHandler(send_message)
    send_messages(bot, [
        'start',
    ...
    assert send_message.call_args_list == create_list_of_calls(mocker, [
        ...
        'X.X\nOO.\n..X',
        'X.X\nOOO\n..X',
        'Game is finished, O wins'
    ])


def test_draw_finished(mocker: pytest_mock.MockFixture) -> None:
    send_message = mocker.stub(name='send_message_stub')
    bot = TicTacToeUserHandler(send_message)
    send_messages(bot, [
        'start',
