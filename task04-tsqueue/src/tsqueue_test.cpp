...
TEST_CASE("ThreadsafeQueue multithreaded ping-pong") {
    ...
    auto pinger = [](void *_qs) -> void * {
        ...
        for (int i = 0; i < PING_PONGS; i++) {
            int tmp = i;
            threadsafe_queue_push(&qs[0], &tmp);
            CHECK(threadsafe_queue_wait_and_pop(&qs[1]) == &tmp);
            CHECK(tmp == i + 1);
        }
        return nullptr;
    };
    auto ponger = [](void *_qs) -> void * {
        ThreadsafeQueue *qs = static_cast<ThreadsafeQueue *>(_qs);
        for (int i = 0; i < PING_PONGS; i++) {
            int *tmp = (int *)threadsafe_queue_wait_and_pop(&qs[0]);
            (*tmp)++;
            threadsafe_queue_push(&qs[1], (void *)tmp);
        }
        return nullptr;
    };
...
void *consumer_try(void *_q) {
    ...
    void *tmp = nullptr;
    for (int i = 0; i < ELEMENTS_PER_THREAD; i++) {
        REQUIRE(threadsafe_queue_try_pop(q, &tmp) == false);
        REQUIRE(tmp == nullptr);
    }
    return nullptr;
}
