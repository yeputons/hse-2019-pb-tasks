...
TEST_CASE("ThreadsafeQueue multithreaded ping-pong") {
    ...
    auto pinger = [](void *_qs) -> void * {
        ...
        for (int counter = 0; counter < PING_PONGS; ++counter) {
            int pushed_value = counter;
            threadsafe_queue_push(&qs[0], &pushed_value);
            int *popped_value =
                static_cast<int *>(threadsafe_queue_wait_and_pop(&qs[1]));
            REQUIRE(&pushed_value == popped_value);
            REQUIRE(pushed_value == counter + 1);
        }
        return nullptr;
    };
    ...
    auto ponger = [](void *_qs) -> void * {
        ...
        for (int counter = 0; counter < PING_PONGS; ++counter) {
            ...
            (*popped_value)++;
            threadsafe_queue_push(&qs[1], popped_value);
        }
        return nullptr;
    };
...
void *consumer_try(void *_q) {
    ThreadsafeQueue *q = static_cast<ThreadsafeQueue *>(_q);
    for (int counter = 0; counter < ELEMENTS_PER_THREAD; counter++) {
        void *data;
