...
TEST_CASE("ThreadsafeQueue multithreaded ping-pong") {
    ...
    auto pinger = [](void *_qs) -> void * {
        ...
        for (int i = 0; i < PING_PONGS; i++) {
            int temp = i;
            threadsafe_queue_push(&qs[0], &temp);
            CHECK(threadsafe_queue_wait_and_pop(&qs[1]) == &temp);
            CHECK(temp == i + 1);
        }
        return nullptr;
    };
    ...
    auto ponger = [](void *_qs) -> void * {
        ...
        for (int i = 0; i < PING_PONGS; i++) {
            ...
            (*temp)++;
            threadsafe_queue_push(&qs[1], (void *)temp);
        }
        return nullptr;
    };
...
void *consumer_try(void *_q) {
    ...
    for (int i = 0; i < ELEMENTS_PER_THREAD; i++) {
        void *temp;
        REQUIRE(threadsafe_queue_try_pop(q, &temp));
