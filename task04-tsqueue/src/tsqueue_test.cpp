...
TEST_CASE("ThreadsafeQueue multithreaded ping-pong") {
	...
	auto pinger = [](void *_qs) -> void * {
		...
		for (int i = 0;i < PING_PONGS; ++i)
		{
			...
			int *new_var = static_cast<int *>(threadsafe_queue_wait_and_pop(&qs[1]));
			REQUIRE(&var == new_var);
			REQUIRE(var == i+1);
		}
		return nullptr;
	...
	auto ponger = [](void *_qs) -> void * {
		...
		for (int i = 0;i < PING_PONGS; ++i)
		{
			int *var = static_cast<int *>(threadsafe_queue_wait_and_pop(&qs[0]));
			(*var)++;
			threadsafe_queue_push(&qs[1],var);
		}
		return nullptr;
	};
...
void *consumer_try(void *_q) {
	...
	void *data;
	for (int i = 0; i < ELEMENTS_PER_THREAD; i++) {
		threadsafe_queue_try_pop(q,&data);
