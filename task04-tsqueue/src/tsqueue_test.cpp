...
TEST_CASE("ThreadsafeQueue multithreaded ping-pong") {
    ...
    auto pinger = [](void *_qs) -> void * {
        ...
        for (int i = 0; i < PING_PONGS; i++) {
            int gift = i + 5;
            threadsafe_queue_push(&qs[0], &gift);
            int *return_gift = (int *)threadsafe_queue_wait_and_pop(&qs[1]);
            CHECK(&gift == return_gift);
            CHECK(i + 6 == *return_gift);
        }
        return nullptr;
    };
    ...
    auto ponger = [](void *_qs) -> void * {
        ThreadsafeQueue *qs = static_cast<ThreadsafeQueue *>(_qs);
        for (int i = 0; i < PING_PONGS; i++) {
            int *gift = (int *)threadsafe_queue_wait_and_pop(&qs[0]);
            (*gift)++;
            threadsafe_queue_push(&qs[1], gift);
        }
        return nullptr;
    };
