...
TEST_CASE("ThreadsafeQueue multithreaded ping-pong") {
    ...
    auto pinger = [](void *_qs) -> void * {
        ...
        for (int i = 0; i < PING_PONGS; i++) {
            threadsafe_queue_push(&qs[0], &queue_now);
            queue = threadsafe_queue_wait_and_pop(&qs[1]);
            CHECK(&queue_now == queue);
            CHECK(last_queue + 1 == queue_now);
    ...
    auto ponger = [](void *_qs) -> void * {
        ...
        int *queue;
        for (int i = 0; i < PING_PONGS; i++) {
            queue = static_cast<int *>(threadsafe_queue_wait_and_pop(&qs[0]));
            (*queue)++;
            threadsafe_queue_push(&qs[1], queue);
        }
        return nullptr;
    };
...
void *consumer_try(void *_q) {
    ...
    void *queue;
    for (int i = 0; i < ELEMENTS_PER_THREAD; i++) {
        REQUIRE(threadsafe_queue_try_pop(q, &queue));
    }
    return nullptr;
}
