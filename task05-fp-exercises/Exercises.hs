...
thirdElementOfSecondList xs = tryHead' (tryTail' (tryTail' (tryHead' (tryTail xs))))
                          where tryHead' xs = case xs of
                                                 Just a -> tryHead a
                                                 _      -> Nothing
                                tryTail' xs = case xs of
                                                 Just a -> tryTail a
                                                 _      -> Nothing
...
fifthElement xs = tryHead' (tryTail' (tryTail' (tryTail' (tryTail xs))))
                  where tryHead' xs = case xs of
                                       Just a -> tryHead a
                                       _      -> Nothing
                        tryTail' xs = case xs of
...
quickSort' :: Ord a => [a] -> [a]
quickSort' [] = []
quickSort' xs = left ++ middle ++ right
                where left   = quickSort' $ filter (< head xs) xs
                      middle = filter (== head xs) xs
...
grep' _ _ [] = []
grep' format match (file:files) = format fname (filter match $ snd file) ++ grep' format match files
                 where fname = fst file
...
grepSubstringNoFilename needle = grep' (\_ str -> str) (isSubstringOf needle)
...
grepExactMatchWithFilename needle = grep' (\fname str -> map' ((fname ++ ":") ++) str) (== needle)
